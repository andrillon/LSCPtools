...
function [REMs , false_detection] = detect_REMs_HEOG_VEOG(HEOG, VEOG, param)
...
fprintf('\n>>>> %g horizontal (%2.2f /min) and %g vertical (%2.2f /min) REMs detected\n\n',sum(REMs(:,end)==1),sum(REMs(:,end)==1)/(length(HEOG)/param.Fs/60),sum(REMs(:,end)==2),sum(REMs(:,end)==2)/(length(HEOG)/param.Fs/60))
