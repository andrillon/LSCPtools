function [pV hplot]=simpleTplotm(x,y,newF,colorF,statsF,lineF,transpF,jbFlag,sthFlag,errFlag,lineWidth)
...
if ~iscell(y)
    if ndims(y)==2
        ...
        if size(y,1)==1
            ...
            if jbFlag
                jbfill(x,median(y)+std(y)/sqrt(size(y,1)-1),median(y)-std(y)/sqrt(size(y,1)-1),colorF,colorF,1,transpF);
                hold on;
                hplot(end+1)=plot(x,median(y),'LineWidth',4,'Color',colorF,'LineStyle',lineF);
            else
                hplot(end+1)=plot(x,median(y),'LineWidth',4,'Color',colorF,'LineStyle',lineF); hold on;
                if errFlag
                plot(x,median(y)+std(y)/sqrt(size(y,1)-1),'LineWidth',lineWidth,'Color',colorF,'LineStyle','--');
                plot(x,median(y)-std(y)/sqrt(size(y,1)-1),'LineWidth',lineWidth,'Color',colorF,'LineStyle','--');
                end
            end
            ...
            elseif statsF(1)==2
                ...
                for nC=1:realpos{1}.nclusters
                    if realpos{1}.pmonte(nC)<statsF(2)
                        hold on;
                        plot(x(find(realpos{1}.clusters==nC)),1.1*max(median(y)+std(y)/sqrt(size(y,1)-1))*ones(1,length(find(realpos{1}.clusters==nC))),'Color',colorF,'LineWidth',lineWidth-1,'LineStyle',lineF)
                    end
                end
                ...
                for nC=1:realneg{1}.nclusters
                    if realneg{1}.pmonte(nC)<statsF(2)
                        hold on;
                        plot(x(find(realneg{1}.clusters==nC)),1.1*min(median(y)-std(y)/sqrt(size(y,1)-1))*ones(1,length(find(realneg{1}.clusters==nC))),'Color',colorF,'LineWidth',lineWidth-1,'LineStyle',lineF)
                    end
                end
            elseif statsF(1)==3
                [h pV]=ttest(yo,statsF(3),statsF(2),[],1);
                ...
                if ~isempty(find(h))
                    myaxis=axis;
                    scatter(x(find(h)),zeros(1,sum(h))+1.1*min(median(y)-std(y)/sqrt(size(y,1)-1)),'Marker','*','MarkerFaceColor',colorF,'MarkerEdgeColor',colorF,'LineWidth',lineWidth);
                end
            end
        end
    else
    ...
    for nc=1:length(y)
        ...
        if ndims(ytemp)==2
            ...
            if size(ytemp,1)==1
                ...
                if jbFlag
                    jbfill(x,median(ytemp)+std(ytemp)/sqrt(size(ytemp,1)-1),median(ytemp)-std(ytemp)/sqrt(size(ytemp,1)-1),colorF(nc,:),colorF(nc,:),1,transpF);
                    hold on;
                    hplot(end+1)=plot(x,median(ytemp),'LineWidth',lineWidth-1,'Color',colorF(nc,:),'LineStyle',lineF{nc});
                else
                    hplot(end+1)=plot(x,median(ytemp),'LineWidth',lineWidth-1,'Color',colorF(nc,:),'LineStyle',lineF{nc}); hold on;
                    plot(x,median(ytemp)+std(ytemp)/sqrt(size(ytemp,1)-1),'LineWidth',1,'Color',colorF(nc,:),'LineStyle','--');
                    plot(x,median(ytemp)-std(ytemp)/sqrt(size(ytemp,1)-1),'LineWidth',1,'Color',colorF(nc,:),'LineStyle','--');
                end
            end
        else
