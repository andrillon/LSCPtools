...
for nS=1:length(subject_id)
    ...
    REMstart=find(REMscaracs.start);
    ...
    for rem=1:length(REMstart)
        ...
        trialrem=find(remtime-(flankSize+smallEpoch*2.1)*fs>trials_phase.trials(:,1) & remtime+ceil((flankSize+smallEpoch*2.1)*fs)<trials_phase.trials(:,2));
        if sum(trialrem)==0
            continue
        end
        ...
        redo=1;
        while redo
            pickrandtrial=ceil(rand*length(trialsREM));
            remtrial=trialsREM(pickrandtrial);
            trialtime=(flankSize+smallEpoch*2.1)*fs+trials_phase.trials(remtrial,1)+1:trials_phase.trials(remtrial,2)-1-(flankSize+smallEpoch*2.1)*fs;
            remtime=trialtime(ceil(rand*length(trialtime)));
            if ismember([REMtime(:)*fs;remtimerand(:)],trialtime);%Hz
                redo=1;
            else
                redo=0;
            end
        ...
        clear filtData
        nrtrial_REM=remtrial;
        nE=trials_phase.id(remtrial);
        %pick all trials scored as REM
        ...
        remtimerand(ncountREM(nS))=remtime;
        ...
        for nSeg=1:length(begS)
            ...
            for nLag=1:length(lagvec)
                ...
                if SidenE=='L' %R
                    ...
                    tem_att{nS}(ncountREM(nS),nLag,nSeg)=test_rho1;
                    tem_ign{nS}(ncountREM(nS),nLag,nSeg)=test_rho2;
                elseif SidenE=='R' %L
                    %                         side_Tale_Sleep(nS,nEc)=2;
                    tem_att{nS}(ncountREM(nS),nLag,nSeg)=test_rho2;
                    tem_ign{nS}(ncountREM(nS),nLag,nSeg)=test_rho1;
                end
            end
    ...
    %nrsubject,start,end,burst?,left?,right?,up?,down?
    REM_compute{nS}=[REM_trial{nS}(:),REM_valid{nS}(:),REMtime(REM_valid{nS})',remtimerand',ismember(REM_valid{nS},REMscaracs.burst)',ismember(REM_valid{nS}-1,[0,REMscaracs.end])',ismember(REM_valid{nS},REMscaracs.left)',ismember(REM_valid{nS},REMscaracs.right)',ismember(REM_valid{nS},REMscaracs.up)',ismember(REM_valid{nS},REMscaracs.down)'];
