%% % parameters for reconstruction without individual Lags
...
for nS=1:length(subject_id)
    ...
    try
        ...
        save([model_path subName,num2str(filterband(1)),'to',mat2str(lagvec),'.mat'],'g')
    end    
    ...
    for nT=1:length([trials_st(4).id])
        ...
        elseif strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'R')
            tem_att{nS,nSta}(count(nS,nSta),1)=test_rho2;
            tem_ign{nS,nSta}(count(nS,nSta),1)=test_rho1;
        end
        ...
        if ~isempty(begS)
            for nSeg=1:length(begS)
                ...
                if strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'L')
                    tem_att_Seg{nS,nSta}(count(nS,nSta),1,nSeg)=test_rho1;
                    tem_ign_Seg{nS,nSta}(count(nS,nSta),1,nSeg)=test_rho2;
                elseif strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'R')
                    tem_att_Seg{nS,nSta}(count(nS,nSta),1,nSeg)=test_rho2;
                    tem_ign_Seg{nS,nSta}(count(nS,nSta),1,nSeg)=test_rho1;
                end
            end
        end
