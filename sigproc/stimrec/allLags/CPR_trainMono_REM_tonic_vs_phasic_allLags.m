...
for nS=1:length(subject_id)
    ...
    if size(REMtonic_stim,1)>0
        [gdum,rstim_tonic] = StimuliReconstruction ([], [], REMtonic_eeg, g, lagvec);
        [rho_real_tonic(nS), ~]=corr(rstim_tonic',REMtonic_stim(:,1));
        [rho_jab_tonic(nS), ~]=corr(rstim_tonic',REMtonic_stim(:,2));
    end
    if size(REMphasic_stim,1)>0
        [gdum,rstim_phasic] = StimuliReconstruction ([], [], REMphasic_eeg, g, lagvec);
        [rho_real_phasic(nS), ~]=corr(rstim_phasic',REMphasic_stim(:,1));
        [rho_jab_phasic(nS), ~]=corr(rstim_phasic',REMphasic_stim(:,2));
    end
