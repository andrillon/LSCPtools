...
for nSta=1:length(NameStages)
    for nS=1:length(subject_id)
        if count(nS,nSta)>=4
            rho_att_bySub_mean(nS,nSta)=mean(tem_att{nS,nSta});
            rho_ign_bySub_mean(nS,nSta)=mean(tem_ign{nS,nSta});
            dec_bySub_mean(nS,nSta,:)=100*mean(tem_att{nS,nSta}>tem_ign{nS,nSta});
            ...
            if ~isempty(begS_half)
                for nSeg=1:length(begS_half)
                    rho_att_bySub_half_mean(nS,nSta,nSeg)=mean(tem_att_half{nS,nSta}(:,nSeg));
                    rho_ign_bySub_half_mean(nS,nSta,nSeg)=mean(tem_ign_half{nS,nSta}(:,nSeg));
                    dec_bySub_half_mean(nS,nSta,nSeg)=100*mean(tem_att_half{nS,nSta}(:,nSeg)>tem_ign_half{nS,nSta}(:,nSeg));
            ...
            if ~isempty(begS)
                for nSeg=1:length(begS)
                    rho_att_bySub_Seg_mean(nS,nSta,nSeg)=mean(tem_att_Seg{nS,nSta}(:,:,nSeg));
                    rho_ign_bySub_Seg_mean(nS,nSta,nSeg)=mean(tem_ign_Seg{nS,nSta}(:,:,nSeg));
                    dec_bySub_Seg_mean(nS,nSta,nSeg)=100*mean(tem_att_Seg{nS,nSta}(:,:,nSeg)>tem_ign_Seg{nS,nSta}(:,:,nSeg));
