...
Numstages={[0,1],[0,1]};%scoring reference for each stage
maxdiff=[];
...
tem_att_half=cell(length(subject_id),length(Numstages));
tem_ign_half=cell(length(subject_id),length(Numstages));
...
for nS=1:length(subject_id)
    ...
    for nE=trials_training
        ...
        for nE2=setdiff(trials_training,nE)
            
            %saveName for data
            saveName=sprintf('%s_T%02.0f_ButFilt_%g_%gHz_filtData',SubID,nE2,filterband(1),filterband(2));
        ...
        %             if strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'L')
        tem_att{nS,nSta}(count(nS,nSta),1)=test_rho1;
        tem_ign{nS,nSta}(count(nS,nSta),1)=test_rho2;
        %             elseif strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'R')
        ...
        if ~isempty(begS_half)
            for nSeg=1:length(begS_half)
                ...
                [gdum,rstimdum] = StimuliReconstruction ([], [], filtData.eeg(thisSeg,:), g, -lagvec);
    ...
    save([savePath,filesep,'leaveoneout',num2str(filterband(1)),'to',num2str(filterband(2)),'.mat'],'tem_att','tem_att_half','tem_ign_half','tem_ign','tem_att_Seg','subject_id','tem_ign_Seg','begS','endS','begS_half','endS_half','count','lagvec','Numstages','NameStages','side_Tale','nE_id','userID','Expe','channels_to_reject')%% plot for subjects
end
