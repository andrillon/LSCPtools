...
%         phasicREMs=sum(trials_phase.trials(nT,1)<[pREM(nS).time]*500 & trials_phase.trials(nT,2)>[pREM(nS).time]*500);
        meanscoring=trials_st(4).tagSleepScoringAv(nT);
        count(nS)=count(nS)+1;
        nrREMtrial{nS}(count(nS))=meanscoring;
...
%                 for nLag=1:length(lagvec)
                    ...
                    if strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'L')
                        tem_att_half{nS}(count(nS),nSeg)=test_rho1;
                        tem_ign_half{nS}(count(nS),nSeg)=test_rho2;
                    elseif strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'R')
                        tem_att_half{nS}(count(nS),nSeg)=test_rho2;
                        tem_ign_half{nS}(count(nS),nSeg)=test_rho1;
                    end
                end
            end
%         end
...
%                     [gdum,rstimdum] = StimuliReconstruction ([], [], filtData.eeg(thisSeg,:), g(:,nLag), lagvec(nLag));
                    ...
                    if strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'L')
                        tem_att_Seg{nS}(count(nS),nSeg)=test_rho1;
                        tem_ign_Seg{nS}(count(nS),nSeg)=test_rho2;
                    elseif strcmp(SubjectBehavData.TrialsCaracs(nE).tSide,'R')
                        tem_att_Seg{nS}(count(nS),nSeg)=test_rho2;
                        tem_ign_Seg{nS}(count(nS),nSeg)=test_rho1;
                    end
%                 end
            end
        end
